...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val answers = postings
      .filter(x => x.postingType == 2)
      .map(p => (show(p.parentId), p))
  ...
  grouped
      .flatMap(_._2)
      .groupByKey()
      .map{case (a,b) => (a, answerHighScore(b.toArray))}
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    val vectors= scored
      .map{case (a,b) => (firstLangInTag(a.tags,langs).get*langSpread,b)}
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val groupedBySize = vs
        .map(_._1/langSpread)
        .groupBy(identity)
        .mapValues(_.size)

      val langLabel: String   = langs(groupedBySize.maxBy(_._2)._1) // most common language in the cluster
      val langPercent: Double = groupedBySize(groupedBySize.maxBy(_._2)._1)*100.0d/vs.size // percent of the questions in the most common language
      ...
      val medianScore: Int    = if (clusterSize%2 == 0)
        ...
        else sortedScores(middle)// rounds down
